...
fun PaymentScreen(
    ...
    val paymentMethod = DataDummy.dummyPaymentMethod.map { dummyItem ->
        PaymentMethod(
            icon = dummyItem.icon,
            name = dummyItem.name
        )
...
private fun PaymentScreenPreview() {
    VibeStoreTheme(dynamicColor = false) {
        // Map the dummy data to the correct model type
        val paymentMethods = DataDummy.dummyPaymentMethod.map { dummyItem ->
            PaymentMethod(
                icon = dummyItem.icon,
                name = dummyItem.name
            )
        ...
        PaymentContent(
            state = paymentMethods, // Now passing the correctly typed list
            latestCheckout = Checkout(
                id = 0,
                receiverName = "",
                receiverAddress = "",
                ...
                shippingDescription = "",
                paymentMethod = "",
                totalPrice = 120.50, // Example value
                coupon = "",
                quantity = ""
            ),
            onChoosePaymentMethod = {},
