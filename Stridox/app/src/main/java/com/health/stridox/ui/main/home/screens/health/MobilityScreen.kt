...
private val mobilityWeekSample = listOf(75f, 78f, 80f, 76f, 79f, 82f, 77f)
...
fun MobilityScreen(
    ...
    val panelRounded = Color(0xFFB39DDB).copy(alpha = 0.16f) // Purple/Lilac accent base
    val accent = Color(0xFFB39DDB)
    val muted = Color(0xFF8C98A8)
    val colorLine = Color(0xFFB39DDB) // main line
    ...
    // --- Dummy datasets ---
    // Week: 7 points
    val weekPoints = mobilityWeekSample

    // Day: 24 points (simulate hourly data)
    val dayPoints = List(24) { idx ->
        (70f + Random(idx).nextFloat() * 15f)
    }
            ...
            Card(
                ...
                Column(modifier = Modifier.padding(20.dp)) {
                    Row(verticalAlignment = Alignment.CenterVertically) {
                        Column {
                            Text("Current Mobility", color = muted)
                            Spacer(modifier = Modifier.height(6.dp))
                            Row(verticalAlignment = Alignment.Bottom) {
                                Text(
                                    text = currentVal.toInt().toString(),
                                    color = Color(0xFF9575CD), // Slightly darker for readability
                                    fontSize = 44.sp,
                                    fontWeight = FontWeight.ExtraBold
                                )
                                Spacer(modifier = Modifier.width(8.dp))
                                Text(
                                    "", // Mobility might not have a specific unit, or maybe a score
                                    color = muted,
                                    fontSize = 18.sp
                                )
                            }
